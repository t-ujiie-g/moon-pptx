...
### Phase 5 — Builder API: create from scratch *(in progress)*
...
- [x] **Phase 5a — `Presentation` façade (open / save / typed accessors)** *(complete)* — `src/presentation/` sub-package: `Presentation::open(bytes)` wraps `@opc.Package::open` and re-wraps `OpcError` as `PptxError::OpcFailure`; `Presentation::save()` passes through `Package::to_bytes`. Typed accessors `slides() / themes() / slide_masters() / slide_layouts() / notes_slides() / comment_lists() / comment_authors()` walk parts by content type (lazy parse on demand). All seven sub-package errors flatten into `PptxError` via per-package `*Failure(String)` variants. Slide ordering is package storage order for now; presentation-xml-driven order lands in Phase 5b. 8 new tests, 504 total × 4 backends.
- [ ] Embedded blank-template `.pptx` bytes (`template/blank.pptx`) — superseded by Phase 5b's programmatic default (build the minimal deck via the Phase 4 writers instead of embedding bytes).
...
- [ ] `Presentation::new() -> Presentation` *(Phase 5b)*
- [ ] `Presentation::slides() -> SlideCollection` — Phase 5a returns `Array[Slide]`; ordered `SlideCollection` lifts the abstraction in Phase 5b
