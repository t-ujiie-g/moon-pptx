...
| SmartArt build | ❌ identification only | ❌ | △ D1 slices 1–3 (`add_smartart_mut` — `list` / `process` / `cycle` / `pyramid` / `org_chart` / `hierarchy` + cached drawing) ⭐ | ⏳ v0.5 (D1 matrix / relationship) |
...
`feature/v0_5`, plus **D1 slices 1–3** (SmartArt ⭐ — the four *flat*
families `list` / `process` / `cycle` / `pyramid` plus the two
hierarchical families `org_chart` / `hierarchy`, with the full
five-part DiagramML + cached drawing). **D1's `matrix` /
`relationship` families are the remaining v0.5.0 work.** 1060 tests ×
4 backends.
...
🟡 **D1 — SmartArt builder** ⭐ headline feature *(slices 1–3 — flat families + hierarchies: list / process / cycle / pyramid / org-chart / hierarchy — landed 2026-06-12)*
