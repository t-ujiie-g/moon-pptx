...
| SmartArt build | ❌ identification only | ❌ | △ D1 slices 1–2 (`add_smartart_mut` — `list` / `process` / `cycle` / `pyramid` + cached drawing) ⭐ | ⏳ v0.5 (D1 hierarchical families) |
...
Status (2026-06-11): D2 (animation DSL ⭐), D8 (chart-option
validation), and C5 (YouTube / URL video embed) landed on
`feature/v0_5`, plus **D1 slices 1–2** (SmartArt ⭐ — the four *flat*
families `list` / `process` / `cycle` / `pyramid` with the full
five-part DiagramML + cached drawing). **D1's hierarchical families
(org-chart / hierarchy / matrix / relationship) are the remaining
v0.5.0 work.** 1057 tests × 4 backends.
...
🟡 **D1 — SmartArt builder** ⭐ headline feature *(slices 1–2 — flat families: list / process / cycle / pyramid — landed 2026-06-11)*
...
- **Consequences**: SmartArt is creatable in moon-pptx — a feature neither competitor offers — with zero parser/writer churn and lossless round-trip preserved. The trade-off is that PowerPoint's *re-layout on edit* depends on the minimal layout defs (live-PowerPoint verification deferred to the sample-deck pass), and child nodes / per-node styling are carried in the model but not yet emitted until the hierarchical slices. Adding a family is a new `SmartArtKind` + its drawing layout + (optionally) a richer data-model shape — no new parts or relationship plumbing.
