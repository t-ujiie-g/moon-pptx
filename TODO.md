...
| Run-level: text outline (`<a:ln>`) / non-solid text fill | △ | ✅ `outline` | △ extension-only (solid fill ✅) | ⏳ v0.6 (F3 follow-up) |
| Run-level: text glow / shadow effects (`<a:effectLst>`) | ❌ | ✅ `glow`/`shadow` | △ extension-only | ⏳ v0.6 (F3 follow-up) |
...
Status (2026-06-16): **F1 + F2 landed; F3 slice 1 landed (kerning +
highlight); F3 outline/effects/fill + F4 + F5 remain.** Priority order
F1 → F2 → F3 → F4 → F5.
...
🟡 **F3 — Run-level rich formatting (highlight / kerning / text outline / text fill / text effects)**
  ...
  - 🔴 **Follow-up — text outline (`<a:ln>`) + text effects (`<a:effectLst>`)**: these reuse the shape-level `@oxml.parse_stroke` / `parse_effect_list`, but **those parsers are strict** — `OuterShadow` models `blurRad`/`dist`/`dir` as *required* (non-optional) and raises when absent, though the spec defaults them to 0. Routing a run's `<a:effectLst>` through that parser would **raise on spec-valid-but-minimal input that previously round-tripped via `extension`** — a robustness regression (the streaming reader can't cleanly fall back to `extension` mid-parse). So this slice waits on making the `@oxml` effect/stroke parsers *lift-safe* (default the optional attrs instead of raising), a separate change with its own shape round-trip implications.
