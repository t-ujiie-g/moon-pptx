...
| SmartArt build | ❌ identification only | ❌ | ✅ D1 (`add_smartart_mut` — all 8 families: list / process / cycle / pyramid / org_chart / hierarchy / matrix / relationship + cached drawing) ⭐ | — |
...
8. **SmartArt creation** (v0.5 D1) — all eight families (list / process / cycle / pyramid / org-chart / hierarchy / matrix / relationship) build a full five-part DiagramML graphic with a self-computed cached drawing. python-pptx can only *identify* SmartArt; PptxGenJS can't touch it at all. Plus the typed animation DSL (D2) and slide transitions (D3) neither competitor exposes above the XML level.
...
🟢 **D1 — SmartArt builder** ⭐ headline feature *(slices 1–4 — all eight families: list / process / cycle / pyramid / org-chart / hierarchy / matrix / relationship — landed 2026-06-12)*
  ...
  - **Follow-ups (documented, additive — D1's headline scope is met)**: parent→child **connector lines** in the cached drawing (the data model already encodes the edges; PowerPoint draws connectors on its first re-layout); per-node styling (the roadmap's `Node.style`); and live-PowerPoint verification in the sample-deck pass (verified here by save→reopen + XML well-formedness across 4 backends).
