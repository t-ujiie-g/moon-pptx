...
| License | Apache-2.0 |
| MoonBit toolchain | `moon 0.1.20260827` or newer (raised 2026-09-01 — the tree now uses the `StringBuilder(size_hint=…)` constructor and `extend T with Show::{to_string}` declarations; the previous `0.1.20260729` floor still carries `String::contains` / `find`, view slicing `s[a:b]`, and `StringView` patterns) |
