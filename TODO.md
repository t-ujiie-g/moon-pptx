...
### Phase 3 — Read path: parse OOXML to model *(read-path slices complete; final polish pending)*
...
- [ ] **Phase 3f — Lossless preservation (ADR-004)** *(in progress)*
  ...
  - [ ] **3f2**: Wire `extension : Array[@xml.XmlElement]` into `@slide.{Slide, AutoShape, Picture, Connector, GroupShape}` (proof-of-concept rollout): replace top-level `skip_subtree` call sites with `collect_subtree` and migrate `Shape::Unknown(String)` to `Shape::Unknown(@xml.XmlElement)` so `<p:graphicFrame>` / `<p:contentPart>` round-trip the full subtree.
  - [ ] **3f3**: Roll out `extension` to remaining model nodes — `@theme` (Theme / ColorScheme / FontScheme), `@slide_master` (SlideMaster / SlideLayout), `@slide` text-side (TextBody / Paragraph / Run / Body+ListStyle / property bags), and the `@oxml` ADTs (Fill / Stroke / EffectList / Color modifier tail) where unknown children currently get dropped. Tick off the Phase 3 lossless-preservation checkbox once complete.
...
- [ ] **Unknown-element preservation strategy** decided and implemented
      (recommend: every model node carries `extension : Array[XmlElement]`) — *Phase 3f is the implementation; tick once 3f3 lands.*
