...
| Comments | ✅ | ❌ | ✅ read+write | — |
| Animations | △ XML-level | ❌ | △ D2 entrance/exit/emphasis/motion-path builder (`Slide::with_animations` + `Timeline`; text builds ⏳) | ⏳ v0.5 (D2) ⭐ |
...
🟡 **D2 — Animation DSL** ⭐ headline feature *(slices 1–3 — entrance/exit + emphasis + motion paths — landed 2026-06-09)*
  ...
  - **Deferred to D2 slice 4+ (the model extends additively)**: **Fly-In**-style positional `<p:anim>` effects, per-paragraph text builds (`<p:bldLst>`), and typed *parsing* of an existing `<p:timing>` into `Timeline`. Slice 1: 12 new tests, additive `.mbti` (+ the `Slide.timing` struct field, like D3's `transition`); 1006 → 1018 × 4 backends.
