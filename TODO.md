...
| Comments | ✅ | ❌ | ✅ read+write | — |
| Animations | △ XML-level | ❌ | △ D2 entrance/exit/emphasis builder (`Slide::with_animations` + `Timeline`; motion paths ⏳) | ⏳ v0.5 (D2) ⭐ |
...
🟡 **D2 — Animation DSL** ⭐ headline feature *(slices 1–2 — entrance/exit + emphasis — landed 2026-06-09)*
  ...
  - **Deferred to D2 slice 3+ (the model extends additively)**: **motion paths** (reusing `CustomGeometry::PathCommand`), **Fly-In**-style positional `<p:anim>` effects, per-paragraph text builds (`<p:bldLst>`), and typed *parsing* of an existing `<p:timing>` into `Timeline`. Slice 1: 12 new tests, additive `.mbti` (+ the `Slide.timing` struct field, like D3's `transition`); 1006 → 1018 × 4 backends.
