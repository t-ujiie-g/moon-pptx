...
- [ ] **Phase 3f — Lossless preservation (ADR-004)** *(in progress)*
  ...
  - [ ] **3f3e (follow-up)**: Helper-level skip points inside `@slide.parser.mbt` (`parse_sp_pr` / `parse_grp_sp_pr` / `parse_nv_sp_pr` / `parse_nv_pr` / `parse_nv_cxn_sp_pr` / `parse_xfrm`) still drop `<a:scene3d>` / `<a:sp3d>` / `<a:extLst>` / `<p:custDataLst>` / `<a:spLocks>` / picture-lock children. Threading the surrounding shape's `extension` array through these helpers wraps up the lossless-preservation promise for the slide subtree.
...
- [x] **Unknown-element preservation strategy** decided and implemented
      (every model node carries `extension : Array[XmlElement]` per ADR-004; see Phase 3f1–3f3 above). One residual: helper-level skips inside `@slide.parser.mbt` (Phase 3f3e follow-up).
